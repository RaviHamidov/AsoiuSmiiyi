    ...
    <div class="col-md-12">
        ...
        <div class="main-card mb-3 card">
            <div class="card-header">Müəllimlər 
                <div class="btn-actions-pane-right">
                    <div role="group" class="btn-group-sm btn-group">
                        <a href="<?php echo base_url('admin_create_item_ad'); ?>">
            ...
            <?php if(empty($items)){ ?>
                ...
                <span>Hər hansı bir məlumat tapılmadı. Məlumat daxil etmək üçün <a href="<?php echo base_url('admin_create_item_ad'); ?>" style="color: green">əlavə et</a></span>
            </div>
            <?php }else{ ?>
                        ...
                        <tbody>
                            ...
                            <?php $itemCount = 1; $for_switch=0; foreach ($items as $item){ $for_switch++; ?>
                                <tr>
                                    ...
                                    <td><?php echo $item->ad_name ." ". $item->ad_surname; ?></td>
                                    <td class="text-center"><?php echo $item->i_c3_name; ?></td>

                                    <td class="text-center">
                                        <?php if($item->ad_img){ ?>
                                            <img width="50" src="<?php echo base_url('upload/administrator/'.$item->ad_img); ?>">
                                        <?php }else{ ?>
                                    ...
                                    <td>
                                        <label class="checkbox-inline">
                                            <input type="checkbox" class="toggle_Switch" id="switchbox<?php echo $for_switch; ?>"
                                            <?php echo ($item->status == 1) ? "checked" : ""; ?> data-toggle="toggle">
                                        </label>
                                    </td>
                                
                                    <td class="text-center">
                                        <a style="text-decoration: none;" href="<?php echo base_url('admin_detail_form_ad/'.$item->ad_id); ?>">
                                            <button type="button" id="PopoverCustomT-1" class="btn btn-outline-primary">Ətraflı</button>
                                        </a>

                                        <a style="text-decoration: none;" href="<?php echo base_url('admin_update_form_ad/'.$item->ad_id); ?>">
                                            <button class="btn-wide btn btn-outline-warning">Redaktə</button>
                                        </a>

                                            <button
                                                    data-url="<?php echo base_url('admin_deleteItem_ad/'.$item->ad_id); ?>"
                                ...
                                <script>
                                    $(document).ready(function() {
                                        ...
                                        $("#switchbox<?php echo $for_switch; ?>").change(function(){
                                            ...
                                            var post_id = <?php echo $item->ad_id; ?>;
                                            var checkbox_val;
                                            if(this.checked) {
                                                checkbox_val = '1';
                                            }else{
                                            ...
                                            $.ajax({
                                                url : "<?php echo base_url('admin_is_active_set_item_ad'); ?>",
                                                type: 'post',
                                                data: dataJson,            
